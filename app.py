    ...
    if isinstance(data, str):
        ...
        for i, row in enumerate(reader):
            ...
            if len(row) >= 6:
                ...
                # Skip if ZIP is not numeric
                if not zipcode[:5].isdigit():
                    continue

                addr = {'street': street, 'city': city, 'state': state, 'zip': zipcode}
...
def geocode_addresses(addresses, center_address=None):
    """Geocode addresses using Census Geocoder with parallel requests"""
    ...
    print(f"Geocoding {len(addresses)} addresses using Census API...")
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        ...
        for future in as_completed(future_to_addr):
            ...
            if completed % 100 == 0 or completed == len(addresses):
                ...
                print(f"  Progress: {completed}/{len(addresses)} ({rate:.1f} req/sec, {elapsed:.1f}s elapsed)")
            ...
            try:
                ...
                if lat and lon:
                    ...
                    geocoded_addr = {**addr, 'lat': lat, 'lon': lon}
                    geocoded.append(geocoded_addr)

                    if center_address and addr['zip'] == center_address['zip'] and addr['street'] == center_address['street']:
    ...
    avg_rate = len(addresses) / total_time if total_time > 0 else 0

    print(f"  Geocoded {len(geocoded)}/{len(addresses)} addresses ({success_rate:.1f}% success)")
    print(f"  Average rate: {avg_rate:.1f} requests/second")

    if failed_count > 0:
        print(f"  {failed_count} addresses failed to geocode")

    return geocoded, geocoded_center
...
def generate_from_file():
    ...
    try:
        ...
        start_time = time.time()
        geocoded, geocoded_center = geocode_addresses(addresses, center_address)
            ...
            if os.path.exists(major_donors_csv):
                ...
                if not major_csv_text:
                    ...
                    if major_addresses:
                        ...
                        major_geocoded, major_geocoded_center = geocode_addresses(major_addresses, major_center)
